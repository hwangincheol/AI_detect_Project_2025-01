...
def detect_objects(image: Image):
    ...
    # 예측 수행 (confidence와 overlap 값을 0~1 범위로 조정)
    results = model.predict(img, confidence=0.4, overlap=0.3).json()

    # 클래스 이름 가져오기
    class_names = model.class_names if hasattr(model, "class_names") else model.classes
    ...
    for prediction in results['predictions']:
        ...
        cv2.putText(img, f"{label} {confidence:.2f}", (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
...
#     results = model.predict(img)                    # 객체 탐지
